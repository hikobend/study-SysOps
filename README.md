...
一部のユーザーにリテンション設定の変更、オブジェクトの削除を許可できる

・コンプライアンスモード

ユーザーが保護されたオブジェクトのバージョンを上書きや削除できない。
...
全てのデバイスで使用するAMIを作成することで、構成の一貫性を維持できる。

・Trusted Advisor

AWSインフラストラクチャサービスを監視し、既知のベストプラクティスと比較して、パフォーマンスやセキュリティの観点からチェックする。

・StorageGateway

頻繁にアクセスするデータはキャッシュでローカルに保持され、　低レイテンシーでのアクセスを実現する。

・CloudHSM

プライベートキーを安全に保管し、暗号化署名オペレーションを行う。
...
読み取り・書き込みアクセス許可をAWSアカウントに付与できる。

・バケットポリシー

バケット内の全てのオブジェクトへのアクセスを特定のIPアドレスに限定できる。バケットの保存されている全てのオブジェクトに対してサーバー側の暗号化を要求するように設定できる。
...
1つのテンプレートから、複数のアカウントおよび複数のリージョンのAWSアカウントにスタックを作成
...
お客様の代わりに自動的に関数をモニタリング。

・EC2AutoScalingライフサイクルフック

EC2インスタンスの終了を制御し、ボリューム上の全ての規制対象データの完全なコピーをS3バケットに送信することができる。

・DisableApiTermination

インスタンスがコンソール・CLI・APIを使用して終了できるかを制御。

・Config

AWSリソースが目的の設定に準拠しているかどうかを評価するために使用されるマネージドルールを提供する。

・procstatプラグイン

特定の実行中のプロセスのCPUメトリクスを収集して送信するために使用できる。

## #96

・ALBのトラブルシューティング

登録されたターゲットが実行中にならない場合、ヘルスチェックに合格していない可能性がある。その予定として、pingパスが存在しないことが考えられる。

・S3オブジェクトロック

ガバナンスモード : 一部のユーザーにリテンション設定の変更、オブジェクトの削除を許可。

コンプライアンスモード : オブジェクトをロックすると、そのリテンションモードを変更することはできず、保持期間おw短縮できない。

・ServiceCatalogの起動制約

製品へのアクセスを制限し、承認されたEC2インスタンス構成飲みを使用してEC2インスタンスの起動を制限する。

・シンプルルーティングポリシー

ドメインで特定の機能を実行する単一のリソースがある場合に使用する。

## #95

・Redisでのクラスターのスケーリング

Redisクラスターにノードグループの数を追加することで、読み取りと書き込みの容量を増やすことができる。

・Trusted Advisor

ユーザーのサービス使用状況をモニタリングし、より効率的なプラン（ベストプラクティス）を提供するサービス

・CloudFormationのスタック間で情報を共有

スタックの出力値をエクスポートする。その結果をインポートする。

・スナップショットライフサイクルの監視

LambdaとEventBridgeを使用すると、プログラムによるイベント通知を処理し、古いスナップショットを削除できる。

## #94

・ドメイン名を使用したトラフィック

Route53にAコードを作成する。CloudFrontディストリビューションを指すエイリアスコードを使用してカスタムドメインを作成できる。

・ユーザー定義のコスト配分タグ

ユーザー定義のタグは、ユーザーが定義、作成し、リソースに適用するタグである。

・AMIの共有

カスタムマネージドキーで暗号化されたAMIのみが共有できる。AMIをコピーして、特定のアカウントと共有する必要がある。また公開しない必要がある。

## #93

・Data Lifecucle Manager

EBSボリュームをバックアップするために作成されたスナップショットの作成・保持・削除を自動化できる。EFSはサポートしていない。

・AWS Backup

AWSサービス全体でデータのバックアップを簡単に一元化し。自動化できる。

・リザーブドインスタンスとのSavingsPlans

RIは個々のメンバーアカウントで購入売る必要がある。RIの割引共有は管理アカウントで無効にする必要がある。
